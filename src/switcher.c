/*
 * switcher.c - High-level layout-switch and caps-toggle action implementations
 *
 * do_switch() sequence:
 ...
 *   5. Convert accordingly.
 *   6. Write converted text to clipboard.
 *   7. Simulate Ctrl+V to paste.
 *   8. Re-select the pasted text (Shift+Left × len).
 *   9. Restore the original clipboard content.
 *
 * do_caps_switch() follows the same sequence but calls toggle_caps()
 * at step 5 instead of convert_layout().
 */
...
void do_caps_switch(void)
{
    ...
    /* --- 3. Copy selection ------------------------------------------------- */
    /* Release Shift/Alt held by the hotkey so Ctrl+C is sent cleanly. */
    release_modifiers();
    Sleep(DELAY_BEFORE_COPY_MS);
    send_ctrl('C');
    Sleep(DELAY_AFTER_COPY_MS);
    ...
    /* --- 5. Toggle caps (invert case of every letter) --------------------- */
    toggle_caps(buf, len);
